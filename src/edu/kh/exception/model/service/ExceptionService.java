...
public class ExceptionService {
	...
	public void ex1() {
		...
		System.out.println("두 정수를 입력받아나누기한 몫을 출력");
		...
		// try{ } : 괄호 내부에 예외가 발생할 가능성이 있는 코드르 작성한 후 시도.
	...
	public void ex3() {
		...
		try {
			...
			// ** 상위 타입의 예외 크래스를 catch문에 작성하면 
			// 다형성 업캐스팅에 의해 모두 잡아서 처리!**
			
			System.out.println("뭔지 모르겠으나 예외가 발생해서 처리함.");
		}
		
		//catch문 연속 작성 시 Exeption 상속 구조를 주의하자! (다형성)
	}
	
	public void ex4() {
		// finally : try 구문에서 예외가 발생 하든말든 무저건 마지막에 수행;
		
		try {
			System.out.println( 4/0 ); // ArithmeticException // 생성
		} catch(ArithmeticException e) {
			...
			System.out.println( e.getMessage() ); // 어떤 발생시 출력된 내용
	...
	public void ex5() {
		...
		//throw : 예외 강제 발생 구문( 현재 매서드에 예외를 던짐 )
	...
	public void methodD() {
		...
		//- RuntimeException과 자식 예외를 제외한 나마지 예외.
		...
		//- 프로그램 수행중 개발자의 코딩 실수 또는 사용자의 잘못된 값 입력으로
		//  흔하게 발생할 수 있는 예외
		// --> if문 같은 단순 코드로 해결 가능 
